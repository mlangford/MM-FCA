...
#Region "formControl"
    ...
    Private Sub btn1Next_Click(sender As System.Object, e As System.EventArgs) Handles btn1Next.Click
        ...
        list_item = m_NWlayers(cboNWdatasetBus.SelectedIndex)
        configObj.NWlayerindexBus = list_item.position
        configObj.NWimpedanceBus = cboCostFieldBus.Text

        'store FCA threshold sizes in configObj
        Try
            configObj.NWdefCutOffCar = Convert.ToDouble(fcaSizeCar.Text)
            configObj.NWdefCutOffBus = Convert.ToDouble(fcaSizeBus.Text)
...
#Region "formLoad configuration"
    ...
    Private Sub s1_frmNetworkLayer_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        'issue warning if not 2+ Network Data layers available
        If m_NWlayers.Count < 2 Then
            MessageBox.Show("**Error** - Insufficient number of Network Datasets present (min. 2)", "**ERROR**", _
                       MessageBoxButtons.OK, MessageBoxIcon.Error)
            cboNWdatasetCar.SelectedIndex = -1
            cboNWdatasetBus.SelectedIndex = -1
        ...
        For Each NWlayer As layerItem In m_NWlayers
            ...
            cboNWdatasetBus.Items.Add(NWlayer.title)
        Next
        cboNWdatasetCar.SelectedIndex = 0
        cboNWdatasetBus.SelectedIndex = 0
...
#Region "display list of Cost fields for a selected NW layer"
    ...
    '### Repeat for BUS network selection

    Private Sub cboNWdatasetBus_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) _
        Handles cboNWdatasetBus.SelectedIndexChanged
        ...
        'store the layer index position for the selected BUS network layer
        selectedNWlayer = m_NWlayers(cboNWdatasetBus.SelectedIndex)
        configObj.NWlayerindexBus = selectedNWlayer.position
        ...
        'get Cost attribute fields for the BUS network, and display in drop-down-list
        cboCostFieldBus.Items.Clear()
        m_costFieldUnitsBus.Clear()
        Try
            For i As Integer = 0 To pNWdataset.AttributeCount - 1
                pNWattribute = pNWdataset.Attribute(i)
                If pNWattribute.UsageType = esriNetworkAttributeUsageType.esriNAUTCost Then
                    cboCostFieldBus.Items.Add(pNWattribute.Name)
                    m_costFieldUnitsBus.Add(pNWattribute.Units.ToString)
                End If
            Next
            If pNWdataset.AttributeCount > 0 Then
                'set Cost field as the first list item
                cboCostFieldBus.SelectedIndex = 0
                lblBusUnits.Visible = True
            Else
                'if no cost field, issue warning message and prevent further progress
                MessageBox.Show("**Error** - no Cost field in selected Mode 2 Network Dataset", _
                                                "**ERROR**", MessageBoxButtons.OK, MessageBoxIcon.Error)
                btn1Next.Enabled = False
            End If
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Codeblock2 Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    'display BUS cost field units
    Private Sub cboCostFieldBus_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) _
    Handles cboCostFieldBus.SelectedIndexChanged

        Dim str() As String = Split(m_costFieldUnitsBus(cboCostFieldBus.SelectedIndex), "esriNAU")
        lblBusUnits.Text = str(1)
