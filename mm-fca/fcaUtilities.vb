...
#Region "distance-decay scaling function"
    ...
    Public Function dist_weighted2(ByRef quantity As Double, ByRef distance As Double, m_configObj As configParams, ByRef mode As String) As Double
        ...
        Else
            cutoff = m_configObj.NWdefCutOffBus
        End If

        If m_configObj.filter = decayType.Classic Then
            'No decay
            dist_weighted2 = quantity
        ElseIf m_configObj.filter = decayType.Linear Then
            'Linear decay 
            dist_weighted2 = quantity * (1.0# - (distance / cutoff))
        ElseIf m_configObj.filter = decayType.Gaussian Then
            'Gaussian decay                           
            dist_weighted2 = quantity * Math.Exp(-(distance ^ 2.0#) / ((cutoff * m_configObj.gaussian_bw / 100.0#) ^ 2.0#))
        ElseIf m_configObj.filter = decayType.Butterworth Then
            'Butterworth decay   
            Dim bw As Double = cutoff * m_configObj.butterworth_bw / 100.0#
            dist_weighted2 = quantity * 1.0# / (1.0# + ((distance / bw) ^ m_configObj.butterworth_pwr))
        End If
