...
#Region "Compute multi-mode accessibility scores"
    ...
    Private Sub btnExecute_Click(sender As System.Object, e As System.EventArgs) Handles btnExecute.Click
        ...
        Dim pNAContextBusCF As INAContext = Nothing
        Dim pNAContextCarOD As INAContext = Nothing
        Dim pNAContextBusOD As INAContext = Nothing
        ...
        Label2.Text = "...loading Mode 1 closest facility data"
        ...
        System.Windows.Forms.Application.DoEvents()
        fcaNAsetClosestsolver2(pNAContextCarCF, configObj, "CAR")
        ...
        'track from layer -> to network layer -> to network dataset
        pLayer = pMap.Layer(configObj.NWlayerindexBus)
        pNWLayer = pLayer
        pNWdataset = pNWLayer.NetworkDataset

        'set NAContext as a ClosestFacility solver for BUS network dataset
        pNAContextBusCF = fcaNAcreateClosestsolver(pNWdataset)

        Label2.Text = "...loading Mode 2 closest facility data"
        System.Windows.Forms.Application.DoEvents()

        'create and configure the Network Analyst layer
        pNALayer = pNAContextBusCF.Solver.CreateLayer(pNAContextBusCF)
        pLayer = pNALayer
        pLayer.Name = "m2FCA_" + pNAContextBusCF.Solver.DisplayName
        pMap.AddLayer(pLayer)
        pNALayer.Expanded = False
        ...
        pLayer = pMap.Layer(configObj.SupplyLayerIndexBus)
        pDisplayTable = pLayer
        loadString = fcaUtilities.getNALoadString(pDisplayTable)
        ...
        'cast to IDisplayTable to access the fields in a joined table
        pLayer = pMap.Layer(configObj.DemandLayerIndexBus)
        pDisplayTable = pLayer
        loadString = fcaUtilities.getNALoadString(pDisplayTable)
        ...
        'solve the network analyst CF problem
        Label2.Text = "solving Mode 2 closest facility analysis..."
        System.Windows.Forms.Application.DoEvents()
        fcaNAsetClosestsolver2(pNAContextBusCF, configObj, "BUS")

        Try

            Dim gpMessagesBusCF As IGPMessages = New GPMessages
            Dim isPartial = pNAContextBusCF.Solver.Solve(pNAContextBusCF, gpMessagesBusCF, Nothing)
            If isPartial Then
                lstOutput.Items.Add("Mode 2: Network Analyst reports that a partial CF solution was found")
            Else
                lstOutput.Items.Add("Mode 2: Network Analyst reports that a full CF solution was found")
            End If
            'display returned messages
            If Not gpMessagesBusCF Is Nothing Then
                For i As Integer = 0 To gpMessagesBusCF.Count - 1
                    Select Case gpMessagesBusCF.GetMessage(i).Type
                        Case esriGPMessageType.esriGPMessageTypeError
                            lstOutput.Items.Add("Error: " + gpMessagesBusCF.GetMessage(i).ErrorCode.ToString() + " " + gpMessagesBusCF.GetMessage(i).Description)
                        Case esriGPMessageType.esriGPMessageTypeWarning
                            lstOutput.Items.Add("Warning: " + gpMessagesBusCF.GetMessage(i).Description)
                        Case Else
                            lstOutput.Items.Add("Information: " + gpMessagesBusCF.GetMessage(i).Description)
                    End Select
                Next
            End If
        ...
        System.Windows.Forms.Application.DoEvents()
        fcaNAsetODsolver2(pNAContextCarOD, configObj, "CAR")
        ...
        pNAContextBusOD = fcaNAcreateODsolver(pNWdataset)

        Label2.Text = "...loading Mode 2 OD matrix data"
        System.Windows.Forms.Application.DoEvents()

        'create and configure the Network Analyst layer
        pNALayer = pNAContextBusOD.Solver.CreateLayer(pNAContextBusOD)
        pLayer = pNALayer
        pLayer.Name = "m2FCA_" + pNAContextBusOD.Solver.DisplayName
        pMap.AddLayer(pLayer)
        pNALayer.Expanded = False
        pMap.MoveLayer(pLayer, pMap.LayerCount - 1) 'move it to the bottom of ToC
        ...
        'solve the network analyst OD Matrix problem
        Label2.Text = "solving Mode 2 OD matrix analysis..."
        System.Windows.Forms.Application.DoEvents()
        fcaNAsetODsolver2(pNAContextBusOD, configObj, "BUS")

        Try
            Dim gpMessagesBusOD As IGPMessages = New GPMessages
            Dim isPartial = pNAContextBusOD.Solver.Solve(pNAContextBusOD, gpMessagesBusOD, Nothing)
            If isPartial Then
                lstOutput.Items.Add("Mode 2: Network Analyst reports that a partial OD solution was found")
            Else
                lstOutput.Items.Add("Mode 2: Network Analyst reports that a full OD solution was found")
            End If
            'display returned messages
            If Not gpMessagesBusOD Is Nothing Then
                For i As Integer = 0 To gpMessagesBusOD.Count - 1
                    Select Case gpMessagesBusOD.GetMessage(i).Type
                        Case esriGPMessageType.esriGPMessageTypeError
                            lstOutput.Items.Add("Error: " + gpMessagesBusOD.GetMessage(i).ErrorCode.ToString() + " " + gpMessagesBusOD.GetMessage(i).Description)
                        Case esriGPMessageType.esriGPMessageTypeWarning
                            lstOutput.Items.Add("Warning: " + gpMessagesBusOD.GetMessage(i).Description)
                        Case Else
                            lstOutput.Items.Add("Information: " + gpMessagesBusOD.GetMessage(i).Description)
                    End Select
                Next
            End If
...
jump1:
        ...
        Try
            ...
            pNewField = New Field
            pNewFieldEdit = pNewField
            pNewFieldEdit.Name_2 = "m2_Choice"
            pNewFieldEdit.Type_2 = esriFieldType.esriFieldTypeSingle
            pNewFieldEdit.Length_2 = 14
            pNewFieldEdit.DefaultValue_2 = 0.0
            pNewFieldEdit.Precision_2 = 12
        ...
        If pCFtable Is Nothing Then
            lstOutput.Items.Add("Error: Unable to access the Mode 1 CF Routes table")
            Exit Sub
        End If
        ...
        'used to store index position of the Incident, Facility, Impedance and Name fields
        Dim idxBUSincident, idxBUSfacility, idxBUSimpedance, idxBUSname As Integer
        Dim impedanceBUS As String = "Total_" & configObj.NWimpedanceBus

        'create dictionaries for associated supplyIDs and network distances
        Dim supplyIDsBUS As New Dictionary(Of Integer, String)
        Dim distancesBUS As New Dictionary(Of Integer, Double)

        'connect to BUS CF results table
        pCFtable = pNAContextBusCF.NAClasses.ItemByName("CFRoutes")
        If pCFtable Is Nothing Then
            lstOutput.Items.Add("Error: Unable to access the Mode 2 CF Routes table")
            Exit Sub
        End If
        'create a cursor to access this table
        pCFCursor = pCFtable.Search(Nothing, True)
        pCFRow = pCFCursor.NextRow

        'get index positions of the fields of interest
        If Not pCFRow Is Nothing Then
            idxBUSincident = pCFRow.Fields.FindField("IncidentID")
            idxBUSfacility = pCFRow.Fields.FindField("FacilityID")
            idxBUSname = pCFRow.Fields.FindField("Name")
            idxBUSimpedance = pCFRow.Fields.FindField(impedanceBUS)
        End If
        ...
        Do Until pCFRow Is Nothing
            ...
            'swap Incident ID and FacilityID values to maintain proper data matching continuity...
            nameStr = pCFRow.Value(idxBUSname)             'access the Name value
            numbers = nameStr.Split("-")                             'split two IDS apart
            pCFRow.Value(idxBUSincident) = numbers(0)      'replace IncidentID with demand point OID
            pCFRow.Value(idxBUSfacility) = numbers(1)         'replace FacilityID with supply point OID 
            pCFRow.Store()                                                  'write values back to table

            'record Supply ID for each Demand ID in the dictionary
            supplyIDsBUS.Add(numbers(0), numbers(1))
            'record Distance for each DemandID in the dictionary
            distancesBUS.Add(numbers(0), pCFRow.Value(idxBUSimpedance))
        ...
        Try
            ...
            Do Until pFeature Is Nothing
                ...
                If supplyIDsCAR.ContainsKey(pFeature.OID) Then
                    ...
                    pResultRow.Value(5) = distancesCAR.Item(pFeature.OID)              'copy supply point distance
                Else
                    'NO: enter values that indicate no supply point found
                    pResultRow.Value(3) = -1
                    pResultRow.Value(5) = -1
                End If
                '** Mode 2 BUS**
                If supplyIDsBUS.ContainsKey(pFeature.OID) Then
                    pResultRow.Value(4) = supplyIDsBUS.Item(pFeature.OID)             'copy supply point OID
                    pResultRow.Value(6) = distancesBUS.Item(pFeature.OID)              'copy supply point distance
                Else
                    pResultRow.Value(4) = -1
                    pResultRow.Value(6) = -1
                End If
        ...
        supplyIDsBUS = Nothing
        distancesBUS = Nothing

        '#### FCA Step1 compute demand totals on each supply point ####'

        ''** Mode 1 CAR **
        Label2.Text = "computing FCA step 1, travel mode 1...'"
        System.Windows.Forms.Application.DoEvents()

        Dim pCursor As ICursor
        Dim pRow As IRow

        'create a dictionary of all CAR demand point volumes
        ...
        'connect to the CAR OD-matrix results table
        Dim pODcarTable As ITable = pNAContextCarOD.NAClasses.ItemByName("ODLines")
        If pODcarTable Is Nothing Then
            lstOutput.Items.Add("Error: unable to access the travel mode 1 OD table")
            Exit Sub
        End If
        'create a cursor to access this table
        ...
        'dictionary for CAR step 1 (total demand volume for each supply point)
        ...
        Try
            ...
            Do Until pRow Is Nothing
                ...
                demandValue = dist_weighted2(demandValue, ntwkDist, configObj, "CAR")
        ...
        'Dim frmDB = New frmDebug
        'frmDB.Show()

        'Dim pair As KeyValuePair(Of Integer, Double)
        'For Each pair In demandCarTotals
        '    ' Display Key and Value.
        '    frmDB.TextBox1.AppendText((pair.Key).ToString + " , " + (pair.Value).ToString + vbCrLf)
        'Next
        'Windows.Forms.MessageBox.Show("OK")

        ''** BUS (Mode 2) **
        Label2.Text = "computing FCA sep 1, travel mode 2...'"
        System.Windows.Forms.Application.DoEvents()

        'create a dictionary of all BUS demand point volumes
        Dim lookup2 As New Dictionary(Of Integer, Double)

        inputFL = pMap.Layer(configObj.DemandLayerIndexBus)
        pDisplayTable = inputFL
        pTable = pDisplayTable.DisplayTable
        pCursor = pTable.Search(Nothing, False)
        pRow = pCursor.NextRow
        Do Until pRow Is Nothing
            lookup2.Add(pRow.OID, Convert.ToDouble(pRow.Value(configObj.DemandVolumeFieldBus)))
            pRow = pCursor.NextRow
        Loop

        'connect to the BUS OD-matrix results table
        Dim pODbusTable As ITable = pNAContextBusOD.NAClasses.ItemByName("ODLines")
        If pODbusTable Is Nothing Then
            lstOutput.Items.Add("Error: unable to access the travel mode 2 OD table")
            Exit Sub
        End If
        'create a cursor to access this table
        pCursor = pODbusTable.Search(Nothing, True)
        pRow = pCursor.NextRow

        'dictionary for BUS Step 1 (total demand volume for each supply point)
        Dim demandBusTotals As New Dictionary(Of Integer, Double)

        'get index positions of the fields of interest
        If Not pRow Is Nothing Then
            idxOriginID = pRow.Fields.FindField("OriginID")
            idxDestinationID = pRow.Fields.FindField("DestinationID")
            idxBUSname = pRow.Fields.FindField("Name")
            idxBUSimpedance = pRow.Fields.FindField("Total_" & configObj.NWimpedanceBus)
        End If
        ...
        Try
            ...
            Do Until pRow Is Nothing
                ...
                'swap OriginID and DestinationID values to maintain proper data matching continuity...
                nameStr = pRow.Value(idxBUSname)             'access the Name value
                numbers = nameStr.Split("-")                         'split two IDS apart
                supplyID = numbers(0)
                demandID = numbers(1)
                pRow.Value(idxOriginID) = supplyID              'replace OriginID with supply point OID
                ...
                ntwkDist = pRow.Value(idxBUSimpedance)

                'use demand ID to lookup the demand value, and scale it
                lookup2.TryGetValue(demandID, demandValue)
                demandValue = dist_weighted2(demandValue, ntwkDist, configObj, "BUS")

                'check if this supply point exists in the dictionary
                If demandBusTotals.ContainsKey(supplyID) Then
                    'YES: add current demand volume to on-going total
                    demandBusTotals.Item(supplyID) += demandValue
                Else
                    'NO: add new dictionay entry with intial volume
                    demandBusTotals.Add(supplyID, demandValue)
                End If
        ...
        '#*#*#*# Now combine the demand totals on each supply point #*#*#*# 

        Dim pair As KeyValuePair(Of Integer, Double)
        Dim demandTotal As New Dictionary(Of Integer, Double)
        For Each pair In demandCarTotals
            demandTotal.Add(pair.Key, pair.Value)
        Next
        For Each pair In demandBusTotals
            If demandTotal.ContainsKey(pair.Key) Then
                demandTotal.Item(pair.Key) += pair.Value
            Else
                demandTotal.Add(pair.Key, pair.Value)
            End If
        Next

        ''display demand counts on each supply point
        'Dim frmDB = New frmDebug
        'frmDB.Show()

        'For Each pair In demandTotal
        '    frmDB.TextBox1.AppendText(pair.Key.ToString + ", " + pair.Value.ToString)
        '    frmDB.TextBox1.AppendText(", " + demandCarTotals.Item(pair.Key).ToString)
        '    frmDB.TextBox1.AppendText(", " + demandBusTotals.Item(pair.Key).ToString + vbCrLf)
        'Next
        ...
        Label2.Text = "computing FCA step 2, travel mode 1...'"
        System.Windows.Forms.Application.DoEvents()
        ...
        'dictionary for CAR step 2 (total step1 scores for each demand point)
        ...
        Try
            ...
            Do Until pRow Is Nothing
                ...
                availabilityScore = supplyValue * configObj.NWscale / demandTotal(supplyID)
                availabilityScore = dist_weighted2(availabilityScore, ntwkDist, configObj, "CAR")
        ...
        Label2.Text = "computing FCA step 2, travel mode 2...'"
        System.Windows.Forms.Application.DoEvents()

        'create a dictionary of all BUS supply point volumes
        Dim lookup4 As New Dictionary(Of Integer, Double)

        inputFL = pMap.Layer(configObj.SupplyLayerIndexBus)
        pDisplayTable = inputFL
        pTable = pDisplayTable.DisplayTable
        pCursor = pTable.Search(Nothing, False)
        pRow = pCursor.NextRow
        Do Until pRow Is Nothing
            lookup4.Add(pRow.OID, Convert.ToDouble(pRow.Value(configObj.SupplyVolumeFieldBus)))
            pRow = pCursor.NextRow
        Loop

        'connect to the BUS OD-matrix results table
        pODbusTable = pNAContextBusOD.NAClasses.ItemByName("ODLines")
        'create a cursor to access this table
        pCursor = pODbusTable.Search(Nothing, True)
        pRow = pCursor.NextRow

        'dictionary for BUS step 2 (total step1 scores at each demand point)
        Dim supplyBusTotals As New Dictionary(Of Integer, destObj)
        ...
        Try
            ...
            Do Until pRow Is Nothing
                ...
                'use supply ID to lookup the supply value, and scale it
                lookup4.TryGetValue(supplyID, supplyValue)
                availabilityScore = supplyValue * configObj.NWscale / demandTotal(supplyID)
                availabilityScore = dist_weighted2(availabilityScore, ntwkDist, configObj, "BUS")

                'check if this demand point already exists in the dictionary
                If supplyBusTotals.ContainsKey(demandID) Then
                    'YES: update information for this Demand
                    supplyBusTotals.Item(demandID).count += 1.0                          'total supply points
                    supplyBusTotals.Item(demandID).sumdist += ntwkDist               'total supply distance
                    supplyBusTotals.Item(demandID).twostep += availabilityScore     'total step1 scores
                Else
                    ...
                    newObj.sumdist = ntwkDist
                    newObj.twostep = availabilityScore
                    supplyBusTotals.Add(demandID, newObj)
                End If
        ...
        Do Until pOTrow Is Nothing
            ...
            If supplyCarTotals.ContainsKey(demandID) Then
                pOTrow.Value(7) = supplyCarTotals.Item(demandID).count
                pOTrow.Value(9) = supplyCarTotals.Item(demandID).sumdist / supplyCarTotals.Item(demandID).count
                pOTrow.Value(11) = supplyCarTotals.Item(demandID).twostep
            End If
            If supplyBusTotals.ContainsKey(demandID) Then
                pOTrow.Value(8) = supplyBusTotals.Item(demandID).count
                pOTrow.Value(10) = supplyBusTotals.Item(demandID).sumdist / supplyBusTotals.Item(demandID).count
                pOTrow.Value(12) = supplyBusTotals.Item(demandID).twostep
            End If
